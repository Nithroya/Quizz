...
const blockQuestions = document.querySelectorAll('.question-block');
let verifTableau = [];
...
function couleursFonction(tabValBool) {
    ...
    for(let i = 0; i < tabValBool.length; i++){
        ...
        if(tabValBool[i] === true){
            blockQuestions[i].style.background = 'lightgreen';   
        } else {
            blockQuestions[i].style.background = '#ffb8b8'; 
            blockQuestions[i].classList.add('echec');   

            setTimeout(() => {
                blockQuestions[i].classList.remove('echec');   
            }, 500);
        }
    }
}
